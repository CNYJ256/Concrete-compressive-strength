...
本文面向混凝土抗压强度预测任务，在 UCI Concrete Compressive Strength 数据集（1030 样本）上，对 `paper1` 复现实验与 `v9` 模型进行了统一框架下的对比研究。`paper1` 侧重单模型路径（AdaBoost/ANN/SVM），而 `v9` 引入龄期分段自适应融合（Age-aware Piecewise Blend），通过 10 折 OOF 预测与约束权重优化，在不同龄期区间学习差异化模型贡献。实测结果显示：`paper1` 的 AdaBoost 在 10 折下达到 $R^2=0.9090$、$RMSE=4.9695$；`v9` 在 10 折下达到 $R^2=0.9488$、$RMSE=3.6996$，对应 $R^2$ 提升 $+0.0398$，RMSE 降低 $1.2699$ MPa（约 $25.55\%$）。研究表明，将龄期从“输入变量”进一步提升为“融合策略条件变量”可显著提高模型泛化性能，并增强工程可解释性。
...
`paper1` 的核心路径是以 AdaBoost 为代表的单模型学习，并与 ANN、SVM 进行对照；`v9` 的核心则是“多基学习器 + 条件化融合”。
...
- Global Blend：$R^2=0.9487$，$RMSE=3.7001$
- Age Piecewise：$R^2=0.9488$，$RMSE=3.6996$
...
| 方法 | 评估协议 | R² | RMSE |
|---|---|---:|---:|
| paper1 AdaBoost | 10-fold CV | 0.9090 | 4.9695 |
| **v9（本文）** | **10-fold CV** | **0.9488** | **3.6996** |
...
1. **精度提升的来源**：`v9` 的关键并非单个基学习器性能突变，而是通过 OOF 级别的权重优化实现误差互补。  
