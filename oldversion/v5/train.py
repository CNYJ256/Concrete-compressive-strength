...
def resolve_paths() -> dict[str, Path]:
    ...
    return {
        ...
        "prev": root / "v2" / "metrics.json",
        "model": root / "v3" / "model.joblib",
        "metrics": root / "v3" / "metrics.json",
    }
...
def main() -> None:
    ...
    try:
        logger.info("===== v3 训练开始：机理交互增强 + HistGradientBoosting =====")
        ...
        model_bundle = {
            ...
            "version": "v3",
            "strategy": "HistGradientBoosting with richer mechanism interactions",
        }
        ...
        logger.info(
            "对比v2: ΔR2=%+.4f, ΔRMSE=%+.4f (正值代表RMSE下降)",
            delta["R2_gain"],
            delta["RMSE_drop"],
        )
